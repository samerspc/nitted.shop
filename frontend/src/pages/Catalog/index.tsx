...
const CatalogPage = () => {
  ...
  const [sizes, setSizes] = useState<{ sizesEu: string[]; sizesUs: string[]; sizesMm: string[] }>({ sizesEu: [], sizesUs: [], sizesMm: [] });
  ...
  useEffect(() => {
    if (gender === 'male' || gender === 'female') {
      ...
      if (selectedSize) {
        if (selectedSize.endsWith('EU')) params.append('sizeEu', selectedSize.replace('EU', ''));
        if (selectedSize.endsWith('US')) params.append('sizeUs', selectedSize.replace('US', ''));
        if (selectedSize.endsWith('MM')) params.append('sizeMm', selectedSize.replace('MM', ''));
      }
      if (selectedSort) {
      ...
      <aside className={styles.aside}>
        {brands.map((item, i) => (
          <button
            key={i}
            className={
              styles.brand_button + (selectedBrand === item ? ' ' + styles.active : '')
            }
            ...
            <div className={styles.sorts_wrapper_options_sorts}>
              {sortOptions.map(option => (
                <div
                  ...
                  style={{ fontWeight: selectedSort.value === option.value ? 'bold' : undefined }}
            ...
            <div className={styles.sorts_wrapper_options_size}>
              {sizes.sizesEu.map(size => (
                <div
                  ...
                  style={{ fontWeight: selectedSize === `${size}EU` ? 'bold' : undefined }}
              ...
              {sizes.sizesUs.map(size => (
                <div
                  ...
                  style={{ fontWeight: selectedSize === `${size}US` ? 'bold' : undefined }}
                  className={styles.sorts_wrapper_options_size__item}
                >
                  {size}US
                </div>
              ...
              {sizes.sizesMm.map(size => (
                <div
                  ...
                  style={{ fontWeight: selectedSize === `${size}MM` ? 'bold' : undefined }}
                  className={styles.sorts_wrapper_options_size__item}
                >
                  {size}MM
                </div>
