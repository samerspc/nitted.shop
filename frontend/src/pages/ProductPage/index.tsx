...
const ProductPage = () => {
  ...
  return (
    <>
      {isMobile ? (
        <p className={styles.closeIcon} onClick={() => window.history.back()}>
          Закрыть
        </p>
      ) : (
        <img
      ...
      <div className={styles.wrapper}>
        <div className={styles.imgs}>
          {product.images.map((item, i) => (
            <img key={i} src={item} alt="" className={i === 0 && isMobile ? styles.p12 : ''} />
          ))}
          {product.images.map((item, i) => (
            ...
            <p>
              {product.inStock ? (
                <>
                  ...
                  <span className={styles.isInStock_text}>Доставка по Москве бесплатно.</span>
                </>
              ) : (
                <>
                  <span className={styles.notInStock}>На заказ.</span>{' '}
                  <span className={styles.isInStock_text}>Доставка в течение 2-3 недель.</span>
                </>
              )}
            </p>
          </div>
